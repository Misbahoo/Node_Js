...
import { chemistryQuestions } from "../questions/chemistryQ";
...
const Chemistry = () => {
  ...
  const answers = chemistryQuestions.map((items, index) => items.answer);

  const handleChange = (e: any) => {
    const numbers = /\d+/g;

    const { value, name } = e.target;
    const currentQuestionNumber = name.slice(name.indexOf("chemistryQ"), 12);
  ...
  const submit = (e: any) => {
    ...
    localStorage.setItem("chemistrySubmitted", "true");
    localStorage.setItem("chemistryAnswers", JSON.stringify(input));
  };
                  ...
                  {question.options.map((item, index) => {
                    return (
                      <div className="mb-3" key={index}>
                        ...
                        <input
                          type="radio"
                          name={`chemistryQ${index1} : ${question.start} ${question.theWord} ${question.end}`}
                          value={item.a}
