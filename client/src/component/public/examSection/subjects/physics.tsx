...
import { physicsQuestions } from "../questions/physicsQ";
...
const Physics = () => {
  ...
  const answers = physicsQuestions.map((items, index) => items.answer);

  const handleChange = (e: any) => {
    const numbers = /\d+/g;

    const { value, name } = e.target;
    const currentQuestionNumber = name.slice(name.indexOf("physicsQ"), 9);
  ...
  const submit = (e: any) => {
    ...
    localStorage.setItem("physicsSubmitted", "true");
    localStorage.setItem("physicsAnswers", JSON.stringify(input));
  };
                  ...
                  {question.options.map((item, index) => {
                    return (
                      <div className="mb-3" key={index}>
                        ...
                        <input
                          type="radio"
                          name={`physicsQ${index1} : ${question.start} ${question.theWord} ${question.end}`}
                          value={item.a}
