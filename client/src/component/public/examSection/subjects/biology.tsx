...
import { biologyQuestions } from "../questions/biologyQ";
...
const Biology = () => {
  ...
  const answers = biologyQuestions.map((items, index) => items.answer);

  const handleChange = (e: any) => {
    const numbers = /\d+/g;

    const { value, name } = e.target;
    const currentQuestionNumber = name.slice(name.indexOf("biologyQ"), 9);
  ...
  const submit = (e: any) => {
    ...
    localStorage.setItem("biologySubmitted", "true");
    localStorage.setItem("biologyAnswers", JSON.stringify(input));
  };
                  ...
                  {question.options.map((item, index) => {
                    return (
                      <div className="mb-3" key={index}>
                        ...
                        <input
                          type="radio"
                          name={`biologyQ${index1} : ${question.start} ${question.theWord} ${question.end}`}
                          value={item.a}
